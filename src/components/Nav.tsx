  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 flex items-center justify-between bg-rhizent-green px-6 py-4 sm:px-10">
        ...
        >
          <Logomark className="h-6 w-6 text-rhizent-gold" />
          <span className="text-sm tracking-[0.3em] text-rhizent-gold">
            RHIZENT CAPITAL
          </span>
        </Link>
      ...
      >
        {links.map((link) => (
          <Link
            ...
            href={link.href}
            onClick={() => setOpen(false)}
            className="font-serif text-3xl text-rhizent-cream transition hover:text-rhizent-gold sm:text-4xl"
          >
            {link.label}
          </Link>
        ))}
